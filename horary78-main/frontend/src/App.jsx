...
function getApiBaseUrl() {
  ...
  if (window.electronAPI?.isElectron) {
    console.log('✅ Detected Electron environment - using 127.0.0.1:5000');
    return 'http://127.0.0.1:5000';
  }
  
  // For web browsers, check environment variables
  const apiUrl = import.meta.env.VITE_API_BASE_URL || window.API_BASE_URL || 'http://localhost:5000';
  console.log('🌐 Using browser mode - API URL:', apiUrl);
  console.log('================================');
  return apiUrl;
}
...
const HoraryAstrologyApp = () => {
  ...
  const checkApiHealth = async (retries = 5) => {
    console.log(`Checking API health (attempt ${6 - retries}/5)...`);
    try {
      ...
      if (retries > 0) {
        ...
        setTimeout(() => checkApiHealth(retries - 1), 2000);
      } else {
